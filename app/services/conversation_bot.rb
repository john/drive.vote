    ...
    case @bot_counter
      when 0..max_counter
        ...
        if results.count == 1
          ...
          @conversation.send("#{from_or_to}_address=".to_sym, result.street_address)
          @conversation.send("#{from_or_to}_city=".to_sym, result.city)
          ...
          formatted = results.first.formatted_address.sub(/, USA/, '')
